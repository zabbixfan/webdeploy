...
function moduleleave2(currvalue) {
	...
	function showResponse(originalRequest)
	{
        res = originalRequest.responseJSON.warpath
        for (i in res){
            for (var j=0;j<res[i]["wars"].length;j++){
                $("#download").append("<option value='"+res[i]["url"]+"'>"+res[i]["wars"][j]+"</option>")
        }
      }
	}
}
...
function commit(){
    ...
    var war=$("#download").find("option:selected").val()+$("#download").find("option:selected").text()
...
function showdetail(currentid){
    ...
    $.ajax({
            ...
            success: function(ret){
                $('.modal-body').html(ret.replace(/\n/g,'<br>'));
            },
            error: function(){
                $('.modal-body').html("内部错误");
            },
    })
}
